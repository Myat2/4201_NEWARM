...
public class Grid extends Network{
   ...
   */
    ...
    public Pose2d findGotoPos(double x, double y, double dist){
        ...
        double cur_x = curPose.getTranslation().getX();
        double cur_y = curPose.getTranslation().getY();
        return findGotoPos(cur_x, cur_y, x, y, dist);
    }
    /**
   ...
   */
    public Pose2d findGotoPos(double cur_x, double cur_y, double x, double y, double dist){
        Tile t;
        ...
        for(int i=0; i<(8); i++) {
            ...
            double pos_x = x + dist*Math.cos(aa);
            double pos_y = y + dist*Math.sin(aa);
            ...
            pos[i] = new Pose2d(pos_x, pos_y, new Rotation2d(aa+Math.PI/2));
