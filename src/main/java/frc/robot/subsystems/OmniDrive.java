...
public class OmniDrive extends SubsystemBase
{
    ...
    // ShuffleboardLayout odometry = Shuffleboard.getTab("OmniDrive")
    // .getLayout("Odometry", BuiltInLayouts.kList).withSize(3, 3);
    // private final NetworkTableEntry D_odoX = odometry.add("odo x", 0).getEntry();
    // private final NetworkTableEntry D_odoY = odometry.add("odo y", 0).getEntry();
    // private final NetworkTableEntry D_odoW = odometry.add("odo w", 0).getEntry();
    ...
    public OmniDrive() {
        ...
        // pidControllers[2] = new PIDController(0.1,1.2,0.00, pid_dT);    //w
        pidControllers[2] = new PIDController(2.0,0.0,0.01, pid_dT);    //w
    ...
    public void setOdometry(Pose2d odo){
        m_odometry.resetPosition(odo);
    }
    ...
    /***
     * Reset the omni drive to initial state. This is useful when disabling/enabling autonomous mode
      */
